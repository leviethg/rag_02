...
class LocalDataIngestion:
    ...
    def _read_file_text(self, input_file: str) -> str:
        """Đọc nội dung file, hỗ trợ PDF, TXT, CSV."""
        ...
        if ext == "pdf":
            try:
                ...
                for page in document:
                    ...
                    text_content += " " + self._filter_text(page_text)
            except Exception as e:
                print(f"[ERROR] Lỗi khi đọc PDF '{input_file}': {e}")

        elif ext == "txt":
            try:
                with open(input_file, "r", encoding="utf-8", errors="ignore") as f:
                    text_content = self._filter_text(f.read())
            except Exception as e:
                print(f"[ERROR] Lỗi khi đọc TXT '{input_file}': {e}")
            ...
            try:
                with open(input_file, "r", encoding="utf-8", errors="ignore") as f:
                    ...
                    text_content = self._filter_text(" ".join(rows))
            except Exception as e:
                print(f"[ERROR] Lỗi khi đọc CSV '{input_file}': {e}")

        else:
            print(f"[WARN] File '{input_file}' có định dạng chưa hỗ trợ: .{ext}")
    ...
    def store_nodes(
        ...
        """Chuyển file → Document → Nodes, hỗ trợ PDF/TXT/CSV."""
        return_nodes = []
        self._ingested_file = []
        ...
        for input_file in tqdm(input_files, desc="Ingesting data"):
            ...
            all_text = self._read_file_text(input_file)
            if not all_text:
                print(f"[WARN] File '{file_name}' rỗng hoặc đọc thất bại.")
                continue
